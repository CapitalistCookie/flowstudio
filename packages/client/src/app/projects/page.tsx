            ...
            {filtered.map((project) => (
              <ContextMenu key={project.id} items={contextMenuItems(project.id)}>
                <button
                  ...
                  className="w-full text-left rounded-xl p-4 transition-colors hover:brightness-110 cursor-pointer"
                  style={{ backgroundColor: 'var(--color-surface)' }}
                >
                  <div
          ...
          <div className="space-y-2">
            {filtered.map((project) => (
              <ContextMenu key={project.id} items={contextMenuItems(project.id)}>
                <button
                  onClick={() => router.push(`/project/${project.id}`)}
                  className="w-full flex items-center gap-4 rounded-lg px-4 py-3 transition-colors hover:brightness-110 cursor-pointer"
                  style={{ backgroundColor: 'var(--color-surface)' }}
                >
