...
import { Video, Volume2, Lock, Eye, Film, Trash2, Scissors, Undo2, Redo2, Copy, Clipboard, Wand2, ChevronDown, ChevronUp, GripHorizontal } from "lucide-react"
...
export function Timeline() {
  ...
  /** Stable order for effect types (used when expanded: one row per type present). */
  ...
  /** Labels for effect blocks (for FX track). */
  ...
  const DEFAULT_EFFECT_DURATION_PX = 5 * PIXELS_PER_SECOND // 5 seconds in base pixels

  /** Display label for a track (e.g. "FX:zoom" -> "Zoom"). */
  const getTrackLabel = useCallback((track: string) => {
