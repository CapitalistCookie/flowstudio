...
interface AutoEnhanceModalProps {
  ...
  onEnhance: (prompt: string, ragResults?: Array<{ start: number; end: number; score: number; mediaName?: string; mediaId?: string; }>) => void
}
...
export function AutoEnhanceModal({ open, onOpenChange, onEnhance }: AutoEnhanceModalProps) {
  ...
  const [ragResults, setRagResults] = useState<Array<{ start: number; end: number; score: number; mediaName?: string; mediaId?: string; }> | null>(null)
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const searchPromises = indexedVideos.map(async (media) => {
        ...
        return (result.data || []).map((clip: { start: number; end: number; score?: number }) => ({
          start: clip.start,
          end: clip.end,
          score: clip.score || 0,
          mediaId: media.id,
