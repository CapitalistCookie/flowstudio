...
/** Modular effect block types attachable to time ranges on the timeline. Implementation is backend. */
export type ModularEffectType =
  | "zoom"
  | "fast_forward"
  | "trim"
  | "cut"
  ...
  | "auto_hide_static_cursor";

/** Persisted data for one modular effect block. startTime/duration in timeline pixels (same as clips). */
export interface EffectBlockData {
  id: string;
  effectType: ModularEffectType;
  startTime: number;
  duration: number;
  /** Optional config for backend; frontend only stores as opaque. */
